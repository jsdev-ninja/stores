...
The authoritative production Firestore ruleset is NOT checked into this repo. No `firebase.json` anywhere declares a `firestore` (rules/indexes) block — `functions/firebase.json` and `apps/store/firebase.json` only define `functions`/`hosting`.

`functions/firestore.rules` (added in the budget rebuild) is an UNTRACKED, NOT-wired file. If someone wires it and runs `firebase deploy --only firestore`, it REPLACES the entire prod ruleset. It only grants reads to a few budget collections + STORES and `allow write: if false` for the catch-all — which would lock out the storefront.
