...
function splitYYMM(yyMM: string): { month: string; year: string } {
  ...
  const correctedYear =
    parseInt(yy, 10) > currentYY ? fullYear - 100 : fullYear;
  ...
  async createPaymentLink(params: TPaymentLinkRequest) {
    try {
      ...
      if (!signResponse.ok) {
        ...
        return {
          success: false,
          errMessage: `HYP returned ${signResponse.status}`,
          paymentLink: null,
          formAction: null,
          formFields: null,
        };
      ...
      if (
        linkData.startsWith("<") ||
        linkData.toLowerCase().includes("<html") ||
        !linkData.includes("=")
      ) {
        logger.write({
          severity: "ALERT",
        ...
        return {
          success: false,
          errMessage: "HYP returned non-signed response",
          paymentLink: null,
          formAction: null,
          formFields: null,
        };
