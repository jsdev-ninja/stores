...
export async function handleOrderUpdated(params: {
  ...
  const paymentCompleted =
    before.paymentStatus !== "completed" && after.paymentStatus === "completed";

  if (paymentCompleted) {
    await appApi.payments.trackPaymentCompleted(after);
    if (after.organizationId) {
      await organizationActionsService
        .onPaymentCompleted(after)
        .catch((err) => {
          logger.write({
            severity: "ERROR",
            message: "organizationActions.onPaymentCompleted failed",
            err,
          });
        });
    }
  ...
  if (deliveryNoteCreated && after.organizationId) {
    await organizationActionsService
      .onDeliveryNoteCreated(after)
      .catch((err) => {
        logger.write({
          severity: "ERROR",
          message: "organizationActions.onDeliveryNoteCreated failed",
          err,
        });
      });
  }
  ...
  if (invoiceCreated && after.organizationId) {
    await organizationActionsService.onInvoiceCreated(after).catch((err) => {
      logger.write({
        severity: "ERROR",
        message: "organizationActions.onInvoiceCreated failed",
        err,
      });
    });
  }
  ...
  if (after.organizationId) {
    ...
    const wasRefunded =
      before.status !== "refunded" && after.status === "refunded";
    if (wasCancelled || wasRefunded) {
      const type = wasCancelled ? "order_cancelled" : "order_refunded";
      budgetWriter
        .onOrderCancelled(after, companyId, storeId, type)
        .catch((err) => {
          logger.write({
            severity: "ERROR",
            message: "budget.onOrderCancelled failed",
            err,
          });
        });
      if (wasCancelled) {
        emitOrderCancelled({ order: after, companyId, storeId }).catch(
          (err) => {
            logger.write({
              severity: "ERROR",
              message: "emit.order_cancelled failed",
              err,
            });
          },
        );
