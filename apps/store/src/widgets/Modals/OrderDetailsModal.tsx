...
const APPROVABLE_PAYMENT_STATUSES: string[] = [
  "pending",
  "pending_j5",
  "completed",
  "external",
...
const statusPill: Record<
  TOrder["status"],
  { variant: PillVariant; emoji: string }
> = {
...
function InfoBanner({
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 text-[12.5px] leading-[1.55] text-start">
        {children}
      </div>
    </div>
  );
...
export function OrderDetailsModal({
  ...
  function actions() {
    ...
    if (
      ...
      APPROVABLE_PAYMENT_STATUSES.includes(order.paymentStatus ?? "")
    ) {
      els.push(
        <Button
          key="approve"
          ...
          isPending={loading}
          onPress={() =>
            run(() => appApi.admin.approveOrder({ order }), "completed")
          }
        >
    ...
    if (order.status !== "cancelled" && order.status !== "completed") {
      els.push(
        <Button
          ...
          isPending={loading}
          onPress={() =>
            run(() => appApi.admin.cancelOrder({ order }), "cancelled")
          }
        >
  ...
  const deliveryNoteNumber = (order as { deliveryNote?: { number?: string } })
    .deliveryNote?.number;
  const invoiceNumber =
    (
      order as {
        ezInvoice?: { doc_number?: string };
        ...
        <Modal.Dialog className="max-h-[85vh] w-full max-w-3xl flex flex-col p-0 overflow-hidden">
          ...
          <Modal.Body className="space-y-4 flex-1 min-h-0 overflow-y-auto px-6 py-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              ...
              <Field
                label={t("ordersPage:orderDetails.paymentMethod", "אופן תשלום")}
              >
                {order.paymentType || "—"}
              </Field>
              <Field
                label={t(
                  "ordersPage:orderDetails.billingAccount",
                  "חשבון לחיוב",
                )}
              >
              ...
              {order.address && (
                <div className="sm:col-span-2">
                  <Field
                    label={t(
                      "ordersPage:orderDetails.delivery.address",
                      "כתובת למשלוח",
                    )}
                  >
                    {[
            ...
            {order.status === "pending" && (
              ...
              >
                {t(
                  "ordersPage:orderDetails.pendingExplain.before",
                  "באישור ההזמנה תיווצר ",
                )}
                <b>
                  {t("ordersPage:orderDetails.deliveryNote", "תעודת משלוח")}
                </b>
                {t(
            ...
            {items.some(
              (i) => i.status === "missing" || i.status === "substituted",
            ) && (
              <InfoBanner
                icon="📦"
                accent="#d4a217"
                gradient="linear-gradient(135deg,#fff7e0,#fdebe0)"
              >
                <b>
                  {t(
                    "ordersPage:orderDetails.fulfillment",
                    "שינויים שהוגדרו בליקוט",
                  )}
                  :
                </b>{" "}
                  ...
                  if (item.status === "missing") {
                    return (
                      <tr key={item.product.id || i} className="bg-[#fef8f5]">
                        ...
                        <td
                          className={`${td} text-start ${struck} text-[12.5px]`}
                        >
                          {item.product.brand || "—"}
                        </td>
                        <td className={`${td} text-center ${struck}`}>
                          {formatter.qty(item.amount)}
                        </td>
                        <td className={`${td} text-end ${struck}`}>
                          {formatter.price(unit)}
                        </td>
                        <td
                          className={`${td} text-end ${struck} text-[12.5px]`}
                        >
                        ...
                        <tr className="bg-[#f3f8f4]">
                          <td className={`${td} text-start`}>
                            ...
                            <ItemPill variant="deliveredInstead">
                              ✓{" "}
                              {t(
                                "ordersPage:orderDetails.deliveredInstead",
                                "נמסר במקום",
                              )}
                            </ItemPill>
                          </td>
                          <td
                            className={`${td} text-start text-[#6b675f] text-[12.5px]`}
                          >
                            {sub.product.brand || "—"}
                          </td>
                          <td className={`${td} text-center`}>
                            {formatter.qty(sub.amount)}
                          </td>
                          <td className={`${td} text-end text-[#6b675f]`}>
                            {formatter.price(sub.price)}
                          </td>
                          <td
                            className={`${td} text-end font-bold text-[#1a1a17]`}
                          >
                    ...
                    {hasBreakdown && (
                      <>
                        <div className="text-[14px] text-[#1a1a17]">
                          {t("ordersPage:orderDetails.totals.items", "פריטים")}:{" "}
                          {formatter.price(itemsSubtotal)}
                        </div>
                        {vat > 0 && (
                          <div className="text-[14px] text-[#1a1a17]">
                            {t("ordersPage:orderDetails.totals.vat", 'מע"מ')}:{" "}
                            {formatter.price(vat)}
                          </div>
                        )}
                        {delivery > 0 && (
                          <div className="text-[14px] text-[#1a1a17]">
                            {t(
                              "ordersPage:orderDetails.totals.delivery",
                              "משלוח",
                            )}
                            : {formatter.price(delivery)}
                          </div>
                        )}
                      </>
                    )}
            ...
            {order.clientComment && (
              <div className="bg-[#fff6d9] p-2.5 text-[13px] text-start">
                <b>{t("ordersPage:orderDetails.notes.title", "הערות")}:</b>{" "}
                {order.clientComment}
              </div>
            )}
            ...
            {deliveryNoteNumber && (
              <div className="bg-[#e3f2e8] p-2.5 text-[13px] text-start">
                <b>
                  📦{" "}
                  {t(
                    "ordersPage:orderDetails.deliveryNoteCreated",
                    "תעודת משלוח שנוצרה",
                  )}
                  :
                </b>{" "}
            ...
            {invoiceNumber && (
              <div className="bg-[#e3eef9] p-2.5 text-[13px] text-start">
                <b>
                  🧾{" "}
                  {t(
                    "ordersPage:orderDetails.invoiceCreated",
                    "חשבונית שנוצרה",
                  )}
                  :
                </b>{" "}
            ...
            {paymentLinkUrl && (
              <div className="bg-[#e3f2e8] p-3 text-[13px] text-start space-y-2">
                <b>
                  🔗 {t("ordersPage:paymentLink.created", "לינק לתשלום נוצר")}:
                </b>
                <div className="text-[12px] text-[#1a1a17]">
                  {t("ordersPage:paymentLink.amount", "סכום החיוב")}:{" "}
                  <b>{formatter.price(order.cart.cartTotal)}</b>
                  {(order.cart.deliveryPrice ?? 0) > 0 && (
                    <>
                      ...
                      <span className="text-[#5a5a55]">
                        (
                        {t(
                          "ordersPage:paymentLink.includesDelivery",
                          "כולל משלוח",
                        )}{" "}
                        {formatter.price(order.cart.deliveryPrice ?? 0)})
                      </span>
                    </>
                  )}
                </div>
