...
export const useAppApi = () => {
	...
	const [loading, setLoading] = useState<
		Partial<
			Record<SubNestedKeys<Omit<typeof api, "loading">>, boolean | undefined>
		>
	>({});
	...
	const api = {
		...
		orders: {
			...
			order: async ({ order }: { order: TOrder }) => {
				...
				const res = await FirebaseApi.api.createOrder({
					order,
					companyId,
					storeId,
				});
				...
				if (!res.success) {
					console.error("Failed to create order via API:", res.error);
					return null; // maintaining legacy behavior of returning falsy or throwing
				}
		...
		admin: {
			...
			getSupplierInvoice: async (id: string) => {
				...
				updateLoading({ "admin.getSupplierInvoice": true });
				const result = await FirebaseApi.firestore.getV2<TSupplierInvoice>({
					collection: FirebaseAPI.firestore.getPath({
						storeId,
						companyId,
						collectionName: "supplierInvoices",
				...
				updateLoading({ "admin.getSupplierInvoice": false });
			...
			removeClientFromOrganization: async (
				clientId: string,
				organizationId?: string,
			) => {
				if (!isValidAdmin) return;
				...
				if (organizationId) {
					updatePayload.organizationIds =
						FirebaseApi.firestore.arrayRemove(
							organizationId,
						) as unknown as string[];
				}
			...
			createOrganization: async (organization: TNewOrganization) => {
				...
				return {
					success: true,
					data: { ...organization, id: result.docId },
				};
			},
			...
			createOrganizationGroup: async (
				organizationGroup: TNewOrganizationGroup,
			) => {
				if (!isValidAdmin || !companyId || !storeId) return;
				...
				const validation =
					NewOrganizationGroupSchema.safeParse(organizationGroup);
				if (!validation.success) {
					logger({
			...
			updateOrganizationGroup: async (
				organizationGroup: TOrganizationGroup,
			) => {
				if (!isValidAdmin || !companyId || !storeId) return;

				const result =
					await FirebaseApi.firestore.update<TOrganizationGroup>(
			...
			listOrganizationGroups: async () => {
				...
				const result =
					await FirebaseApi.firestore.listV2<TOrganizationGroup>({
						collection: FirebaseAPI.firestore.getPath({
							storeId,
							companyId,
							collectionName: "organizationGroups",
		...
		system: {
			...
			auth: {
				logout: async () => {
					...
					actions.dispatch(
						actions.cart.setCart({ cart: null, isReady: false }),
					);
					actions.dispatch(actions.orders.setOrders([]));
					actions.dispatch(
						actions.favoriteProducts.setFavoriteProducts([]),
					);
			...
			categories: {
				get: async () => {
					if (!isValid) return [];
					const res = await FirebaseApi.firestore.get<{
						categories: TCategory[];
					}>(store.id, "categories");
		...
		user: {
			...
			subscriptions: {
				profileSubscribe: () => {
					...
					const unsubscribe =
						FirebaseApi.firestore.subscribeDocV2<TProfile>({
							collection: FirebaseAPI.firestore.getPath({
								collectionName: "profiles",
								storeId,
								companyId,
			...
			getProfileOrganization: async () => {
				...
				if (orgIds.length === 0)
					return { success: true, data: [] as TOrganization[] };
			...
			async createCartFromOrder({ order }: { order: TOrder }) {
				...
				logger({
					severity: "INFO",
					message: "user create cart from order",
					order,
				});
			...
			async addItemToCart({ product }: { product: TProduct }) {
				...
				const isBalasiStorefront =
					store?.id === "balasistore_store" ||
					store?.id === "tester_store";
			...
			async updateCartItemAmount({
				...
				// Update local state
				actions.dispatch(
					actions.cart.updateItemAmount({ product, amount }),
				);
		...
		auth: {
			signup: async (newUser: {
				...
				const res = await FirebaseApi.auth.createUser(
					newUser.email,
					newUser.password,
				);
				if (!res.success) {
				...
				const newProfile = await FirebaseApi.firestore.setV2<
					Partial<TProfile>
				>({
					collection: FirebaseAPI.firestore.getPath({
						collectionName: "profiles",
