...
import { Card, Modal, Select, Tabs, Table, ListBox } from "@heroui/react";
...
export function AdminOrganizationDetailPage() {
  ...
  const [clientFormData, setClientFormData] = useState<ClientFormState>(
  ...
  const [clientSearchResult, setClientSearchResult] = useState<TProfile | null>(
    null,
  );
  const [clientSearchExistingOrg, setClientSearchExistingOrg] =
  ...
  const [organizationGroups, setOrganizationGroups] = useState<
    TOrganizationGroup[]
  >([]);
  const [organizationGroupsLoading, setOrganizationGroupsLoading] =
    useState(false);
  ...
  const validateClientForm = () => {
    ...
    if (!clientFormData.displayName.trim()) {
      errors.displayName = t(
        "admin:organizationsPage.errors.displayNameRequired",
      );
    }
  ...
  const handleRemoveClient = async (client: TProfile) => {
    ...
    try {
      const result = await appApi.admin.removeClientFromOrganization(
        client.id,
        organization.id,
      );
      if (result?.success) {
    ...
    <>
      <div className="flex flex-col gap-1">
        ...
        <Input
          ...
          onChange={(event) =>
            handleClientFormChange("displayName", event.target.value)
          }
        />
      ...
      <div className="flex flex-col gap-1">
        ...
        <Input
          ...
          onChange={(event) =>
            handleClientFormChange("phoneNumber", event.target.value)
          }
        />
      </div>
      <div className="flex flex-col gap-1">
        ...
        <Select
          ...
          onSelectionChange={(key: Key | null) =>
            handleClientFormChange(
              "clientType",
              String(key) as ClientFormState["clientType"],
            )
          }
        >
          <Select.Trigger>
          ...
          <Select.Popover>
            <ListBox>
              ...
              <ListBox.Item
                id="company"
                textValue={t("common:clientTypes.company")}
              >
      ...
      {clientFormData.clientType === "company" && (
        <div className="flex flex-col gap-1">
          ...
          <Input
            placeholder={t(
              "admin:organizationsPage.clientCompanyNamePlaceholder",
            )}
            value={clientFormData.companyName}
            onChange={(event) =>
              handleClientFormChange("companyName", event.target.value)
            }
          />
        </div>
      )}
      <div className="flex flex-col gap-1">
        ...
        <Select
          ...
          onSelectionChange={(key: Key | null) =>
            handleClientFormChange(
              "paymentType",
              String(key) as ClientFormState["paymentType"],
            )
          }
        >
          <Select.Trigger>
          ...
          <Select.Popover>
            <ListBox>
              ...
              <ListBox.Item
                id="external"
                textValue={t("common:paymentTypes.external")}
              >
  ...
  const isBillingNumberTaken = async (
    number: string,
  ...
  const handleEditBillingAccount = async () => {
    ...
    try {
      if (billingFormData.number !== editingBillingAccount.number) {
        if (
          await isBillingNumberTaken(
            billingFormData.number,
            editingBillingAccount.id,
          )
        ) {
          alert("מספר חשבון חיוב כבר קיים בארגון אחר");
          return;
        ...
        billingAccounts:
          organization.billingAccounts?.map(
            (account: { id: string; number: string; name: string }) =>
              account.id === editingBillingAccount.id
                ? billingFormData
                : account,
          ) || [],
      };
  ...
  const handleRemoveBillingAccount = async (accountId: string) => {
    ...
    if (
      window.confirm(t("admin:organizationsPage.confirmDeleteBillingAccount"))
    ) {
      try {
        const updatedOrg = {
          ...organization,
  ...
  const openEditBillingModal = (account: {
    number: string;
    name: string;
    id: string;
  }) => {
  ...
  const openEditOrganizationModal = () => {
    ...
    setOrganizationFormData({
      ...
      paymentType:
        (org as { paymentType?: TPaymentType }).paymentType ||
        ("default" as TPaymentType),
  ...
  return (
    <div className="w-full p-6">
      ...
      >
        <Tabs.List>
          <Tabs.Tab id="details">
            {t("admin:organizationsPage.details" as never)}
          </Tabs.Tab>
          <Tabs.Tab id="orders">
            {t("admin:organizationsPage.orders" as never)}
          </Tabs.Tab>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="text-lg text-start">
                      {t(
                        `common:paymentTypes.${
                          (organization as { paymentType?: string })
                            .paymentType || "default"
                        }` as never,
                      )}
                    </div>
                  ...
                  <div className="flex gap-6">
                    <div className="flex-1 text-center">
                      <p className="text-sm text-gray-500">
                        {t("admin:budget.totalDebits", "סה״כ חיובים")}
                      </p>
                      <p className="text-xl font-bold text-red-600">
                        <Price price={budgetAccount.totalDebits / 100} />
                      </p>
                    </div>
                    <div className="flex-1 text-center">
                      <p className="text-sm text-gray-500">
                        {t("admin:budget.totalCredits", "סה״כ תשלומים")}
                      </p>
                      <p className="text-xl font-bold text-green-600">
                        <Price price={budgetAccount.totalCredits / 100} />
                      </p>
                    </div>
                    <div className="flex-1 text-center">
                      <p className="text-sm text-gray-500">
                        {t("admin:budget.balance", "יתרת חוב")}
                      </p>
                      <p
                        className={`text-xl font-bold ${budgetAccount.balance > 0 ? "text-red-600" : "text-green-600"}`}
                      >
                        <Price price={budgetAccount.balance / 100} />
                      </p>
                    </div>
                  </div>
                </Card.Content>
      ...
      {activeTab === "orders" && (
        <div className="mt-6">
          <Card>
            <Card.Header>
              <Card.Title>
                {t("admin:organizationsPage.ordersHistory" as never)}
              </Card.Title>
            </Card.Header>
            <Card.Content>
              {ordersLoading ? (
                  ...
                  <Table.ScrollContainer>
                    <Table.Content>
                      <Table.Header>
                        ...
                        <Table.Column>
                          {t("ordersPage:columns.date", "Date")}
                        </Table.Column>
                        <Table.Column>
                          {t("ordersPage:columns.sum", "Total")}
                        </Table.Column>
                        <Table.Column>
      ...
      {activeTab === "invoices" && (
        <div className="mt-6">
          <Card>
            <Card.Header>
              <Card.Title>
                {t("admin:organizationsPage.invoicesHistory" as never)}
              </Card.Title>
            </Card.Header>
            <Card.Content>
              {invoicesLoading ? (
            ...
            <Modal.Body>
              <div className="space-y-4">
                <div className="flex flex-col gap-1">
                  ...
                  <Input
                    placeholder={t(
                      "admin:organizationsPage.clientEmailPlaceholder",
                    )}
                    value={clientSearchEmail}
                    onChange={(event) =>
                      setClientSearchEmail(event.target.value)
                    }
                  />
                </div>
                <Button
                ...
                {clientSearchResult && (
                  <div className="border rounded-lg p-4 space-y-1 text-start bg-gray-50">
                    ...
                    <div className="text-xs text-gray-500">
                      {t("common:paymentType")}:{" "}
                      {clientSearchResult.paymentType
                        ? t(
                            `common:paymentTypes.${clientSearchResult.paymentType}`,
                          )
                        : t("common:paymentTypes.none")}
                    </div>
                      ...
                      <div className="mt-3 rounded-md border border-warning-300 bg-warning-50 p-3 text-xs text-warning-600">
                        <div className="font-medium">
                          {clientSearchExistingOrg.id === organization.id
                            ...
                            : t(
                                "admin:organizationsPage.alreadyInOrganization",
                                {
      ...
      <Modal.Backdrop
        ...
        onOpenChange={setIsAddBillingModalOpen}
      >
        <Modal.Container>
          <Modal.Dialog>
            <Modal.Header>
            ...
            <Modal.Body>
              <div className="flex flex-col gap-1">
                ...
                <Input
                  placeholder={t(
                    "admin:organizationsPage.accountNamePlaceholder",
                  )}
                  value={billingFormData.name}
                  onChange={(e) =>
                    handleBillingFormChange("name", e.target.value)
                  }
                />
              </div>
              <div className="flex flex-col gap-1 mt-3">
                ...
                <Input
                  placeholder={t(
                    "admin:organizationsPage.accountNumberPlaceholder",
                  )}
                  value={billingFormData.number}
                  onChange={(e) =>
                    handleBillingFormChange("number", e.target.value)
                  }
                />
              </div>
            </Modal.Body>
            <Modal.Footer>
              <Button
                variant="ghost"
                onPress={() => setIsAddBillingModalOpen(false)}
              >
                {t("common:cancel")}
              </Button>
      ...
      <Modal.Backdrop
        ...
        onOpenChange={setIsEditBillingModalOpen}
      >
        <Modal.Container>
          <Modal.Dialog>
            <Modal.Header>
            ...
            <Modal.Footer>
              <Button
                variant="ghost"
                onPress={() => setIsEditBillingModalOpen(false)}
              >
                {t("common:cancel")}
              </Button>
            ...
            <Modal.Body className="min-h-0 flex-1 overflow-y-auto">
              <div className="grid grid-cols-2 gap-x-4 gap-y-4">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <Input
                    placeholder={t(
                      "admin:organizationsPage.discountPercentagePlaceholder",
                    )}
                    type="number"
                    value={
                      organizationFormData.discountPercentage?.toString() || ""
                    }
                    onChange={(e) =>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <Input
                    placeholder={t(
                      "admin:organizationsPage.nameOnInvoicePlaceholder",
                    )}
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <Input
                    placeholder={t(
                      "admin:organizationsPage.companyNumberPlaceholder",
                    )}
                  ...
                  >
                    ...
                    <Select.Popover>
                      <ListBox>
                        <ListBox.Item
                          id="j5"
                          textValue={t("common:paymentTypes.j5")}
                        >
                          {t("common:paymentTypes.j5")}
                        </ListBox.Item>
                        <ListBox.Item
                        ...
                        <ListBox.Item
                          id="none"
                          textValue={t("common:paymentTypes.none")}
                        >
                          {t("common:paymentTypes.none")}
                        </ListBox.Item>
                      </ListBox>
