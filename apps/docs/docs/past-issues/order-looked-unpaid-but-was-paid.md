...
| **Recurring?** | **Yes — 5 confirmed cases** (all the same customer) |
...
- a **leftover** order — usually `draft` (sometimes `pending`) — holding the early
  version of the basket, showing as unpaid; and
- the **real order** — authorised at checkout, adjusted during packing, then
  charged and invoiced.

The payment and the invoice belong to the **real order**, so the leftover looks
abandoned/unpaid even though the customer paid in full.

## Known occurrences

| When | Leftover (looks unpaid) | Real order (paid) | Amount charged | Document |
|---|---|---|---|---|
...
| 23–28 Apr 2026 | `Iux3HDlnXMjsVY14Eio6` — `pending`, ₪2,446.05 — *cancelled* | `lDBTAKJXQM4lPVxx0gAE` — 40 items | **₪2,485.88** (28 Apr) | EZcount receipt |

All five: same customer (Volumez), same store, same credit card, same shared-cart
fingerprint — the leftover and the paid order point at the **same basket**.

> Note: the leftover is usually `draft`, but the April 2026 case (`Iux3…`) was a
> `pending` order — same pattern, slightly different status.
...
- The leftover order carries the original basket and shows as unpaid, so on its own
  it looks like a lost order.
...
the **real** order, which is marked completed and paid. The leftover has no payment
of its own.
...
- All five leftover orders were **cancelled**. No further action required.

## Worth keeping an eye on

This has happened **at least five times, all for the same customer (Volumez)**.
That strongly suggests something in their checkout flow is creating a duplicate
order. It's worth a proper root-cause look at *why* the duplicate gets created,
plus a periodic sweep for leftover orders that have a paid sibling on the same
basket — they inflate order counts and cause exactly this confusion.

Two practical notes:

- **Timing isn't a reliable signal.** The gap between the leftover and the real order
  ranged from ~1 minute to ~40 minutes. The dependable link is the **shared basket**.
- **The final charge can differ from the amount first authorised** — sometimes higher,
  sometimes lower — because the basket is adjusted during packing.

---

**Reference (for tracing):**

| Case | Leftover | Paid order | Shared basket |
|---|---|---|---|
...
| Apr 2026 | `Iux3HDlnXMjsVY14Eio6` | `lDBTAKJXQM4lPVxx0gAE` | `u2AjCER0uba4l5sy3nCs` |
