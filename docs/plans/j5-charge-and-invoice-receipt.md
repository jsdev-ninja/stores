...
3. **HYP already auto-issues an invoice today — double-invoice risk.** Per Philip: when a credit-card transaction is charged via HYP, **HYP automatically produces an invoice on its (third-party) side** — this is NOT in our code and is **not currently synced with our system / EzCount**. So if EzCount now issues the official חשבונית מס קבלה too, the customer could receive **two invoices**. Decide: turn HYP's invoice off (`SendHesh`) and issue only via EzCount, or keep HYP's and don't issue in EzCount. **Philip: "I'll handle this later."** (`chargeOrder` currently sends `SendHesh: "True"` + `heshDesc`.)
