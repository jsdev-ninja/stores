...
| 3 | **Allocation threshold (₪5,000) on a CONSOLIDATED invoice** | The **sum** can cross ₪5,000 even when individual DNs don't — and `createInvoice` will reject with `allocation_required`. For v1 bulk: **detect per-company `price_total >= 5000`, flag those companies in the wizard, and skip them from the one-click run with a clear "הפק ידנית (נדרשת הקצאה)" message** routing the admin to the existing single-company allocation flow (`AdminOrganizationsPage` / `InvoiceDetailsModal requireAllocation`). Optionally (v1.1) collect an allocation number inline per over-threshold company. This keeps the bulk happy path one-click and respects the ITA gate. |
...
| 5 | **Invoices with no company (B2C) in the list** | **Show them** in the Invoices List (filed under their resolved customer name via the name cascade; the company filter just won't match them). In the **bulk wizard**, group org-less DNs under a "ללא חברה" group but **default them unchecked** — a "consolidated invoice per company" is a B2B concept; B2C invoices are normally 1:1 and the admin should opt in deliberately. (Open sub-question: confirm with owner whether B2C should appear in bulk at all — if not, hide the group.) |
